...
static void beta_scheme(Stack_Array<double>& task_size, ui32 n, double a, double b, ui32 rank) {
	if (rank == 0) {
		...
		//double tmp = -LogGamma(n - j) - LogGamma(j + 1) + LogGamma(j + a) + LogGamma(n - 1 - j + b);
		double tmp = exp(-LogGamma(n) + LogGamma(a) + LogGamma(n - 1 + b));
		task_size[0] = tmp;
		for (ui32 j = 1; j < n; ++j) {
			tmp *= double(n - j - 1) / double(j + 1) * (double(j) + a) / (double(n - j - 1) + b);
			task_size[j] = tmp;
		}
	}
}
...
static void distribute_tasks(Stack_Array<ui32>& task_performer, 
	const Stack_Array<double>& task_size, ui32 rank, ui32 world_size) 
{
...
int main(int argc, char** argv) {	
	...
	try {
		...
		//calculate tasks
		beta_scheme(task_size, L.width(), 0.99, 2.01, rank);
		//distribute tasks
		distribute_tasks(task_performer, task_size, rank, world_size);
		//run
